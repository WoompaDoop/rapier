...
impl PhysicsPipeline {
    ...
    pub fn step(
        ...
        for multibody in &mut multibody_joints.multibodies.iter_mut() {
        ...
        while remaining_substeps > 0 {
            ...
            if remaining_substeps > 0 {
                ...
                for handle in modified_colliders.iter() {
                    ...
                    broad_phase.set_aabb(&integration_parameters, colliders, *handle, aabb);
                }
                modified_colliders.clear();
                // self.clear_modified_colliders(colliders, &mut modified_colliders);
            }
...
mod test {
    ...
    fn kinematic_and_fixed_contact_crash() {
        ...
        let h1 = bodies.insert(0, rb.clone());
        let co = ColliderBuilder::ball(10.0).build();
        colliders.insert_with_parent(0, co.clone(), h1, &mut bodies);

        // The same but with a kinematic body.
        let rb = RigidBodyBuilder::kinematic_position_based().build();
        let h2 = bodies.insert(0, rb.clone());
        colliders.insert_with_parent(0, co, h2, &mut bodies);
    ...
    fn rigid_body_removal_before_step() {
        ...
        // The same but with a kinematic body.
        let rb = RigidBodyBuilder::kinematic_position_based().build();
        let h3 = bodies.insert(0, rb.clone());

        // The same but with a fixed body.
        let rb = RigidBodyBuilder::fixed().build();
        let h4 = bodies.insert(0, rb.clone());
    ...
    fn rigid_body_removal_snapshot_handle_determinism() {
        ...
        let h3b = bodies2.insert(0, rb.clone());
    ...
    fn collider_removal_before_step() {
        ...
        let b_handle = bodies.insert(0, body);
        let collider = ColliderBuilder::ball(1.0).build();
        let c_handle = colliders.insert_with_parent(0, collider, b_handle, &mut bodies);
    ...
    fn rigid_body_type_changed_dynamic_is_in_active_set() {
        ...
        let rb = RigidBodyBuilder::kinematic_position_based()
            .additional_mass(1.0)
            .build();
        let h = bodies.insert(0, rb.clone());
    ...
    fn joint_step_delta_time_0() {
        ...
        let h_dynamic = bodies.insert(0, rb_dynamic.clone());
        ...
        impulse_joints.insert(0, h, h_dynamic, joint, true);
    ...
    fn test_multi_sap_disable_body() {
        ...
        collider_set.insert(0, collider);
        ...
        let ball_body_handle = rigid_body_set.insert(0, rigid_body);
        collider_set.insert_with_parent(0, collider, ball_body_handle, &mut rigid_body_set);
